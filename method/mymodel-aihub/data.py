...
def prepare_data(data_path, max_num, task_type):
    ...
    train_file_list = [data_path + 'train_colloquial.txt', data_path + 'train_literary.txt',
                       data_path + 'dev_colloquial.txt', data_path + 'dev_literary.txt']
    train_label_list = [[0], [1], [0], [1]]
...
class non_pair_data_loader():
    ...
    def create_batches(self, train_file_list, train_label_list, if_shuffle=True):
        ...
        for _index in range(self.num_batch):
            item_data_label_pairs = self.data_label_pairs[
                                    _index * self.batch_size:(_index + 1) * self.batch_size]
            ...
            batch_encoder_length, batch_decoder_length = pad_batch_seuqences(
                item_sentences, self.id_bos, self.id_eos, self.id_unk, self.max_sequence_length,
                self.vocab_size, )
